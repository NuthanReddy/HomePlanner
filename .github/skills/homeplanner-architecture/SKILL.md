...
6. **Preserve intent while regenerating.** Reuse `roomPackProgram` and the manual-layout adapter. Explicitly list unmet requests and invalid saved placements. A named starter is an opt-in proposal, not permission to reset manual layouts, pins, openings or all floors; show changed constraints before applying.
   - New stair requests use an explicit open enclosure profile and a schematic flight/landing/UP symbol; they must not acquire automatic room doors or masonry. Preserve old/missing enclosure intent until explicitly edited. Protect the bounded entry approach, not an entire generic flex-space; the selected-only side guide is advisory.
   - Numeric room programme settings are owned drafts until Apply/Enter. Use `HomePlannerRoomInputs` committed readers/writers in generator adapters and deliberate tile actions, not the visible draft text.
